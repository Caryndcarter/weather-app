...
class Weather {
  ...
  constructor(data: any) {
    ...
    this.tempF = (data.main.temp - 273.15) * 9/5 + 32; // Convert from Kelvin to Fahrenheit
    this.windSpeed = data.wind.speed; // Wind speed in m/s
    this.humidity = data.main.humidity; // Humidity percentage
  }
}
...
class WeatherService {
  ...
  private async readWeatherData(): Promise<Weather[]> {
    try {
      const data = await fs.readFile('db/weatherData.json', 'utf8');
  ...
  async fetchWeatherData(coordinates: Coordinates): Promise<Weather | null> {
    try {
      const response = await fetch(
  ...
  async getWeatherData(cityName: string): Promise<Weather | null> {
    ...
    if (storedCityData) {
      ...
      if (coordinates) {
        ...
        if (newCityData) {
          await this.addWeatherData(newCityData);
          return newCityData;
        }
      }
      return null;
  ...
  private async writeWeatherData(weatherArray: Weather[]): Promise<void> {
    try {
      await fs.writeFile('db/weatherData.json', JSON.stringify(weatherArray, null, 2));
    } catch (error) {
